...
import styles from "./NikiYarInPersonServices.module.scss";
...
export default function NikiYarInPersonServices() {
  ...
  const {
    filteredItems: inPersonItems,
    vipItems,
    discountItems,
    loading,
    refresh: refreshProducts,
  } = useNikiYarProducts({
    deliveryMethod: "1", // فقط سرویس‌های حضوری
    autoRefresh: true,
  });
  ...
  const filteredInPersonItems = useMemo(() => 
  ...
  useEffect(() => {
    ...
    if (inPersonItems.length > 0) {
      const hasVip = inPersonItems.some((i) => i.category_is_vip);
  ...
  return (
    <div className={styles["niki-yar-in-person-services"]}>
      <h1>خدمات حضوری</h1>

      <NikiYarIsVipTab onTabChange={handleTabClick} value={activeTab} />

      <div className={styles["niki-yar-in-person-services__content"]}>
        {loading ? (
          <Text textStyle="14S5" textColor="primary-600">
            در حال بارگذاری...
          </Text>
        ) : inPersonItems.length === 0 ? (
          <NikiYarIsEmpty showAddressSelector={true} />
        ) : filteredInPersonItems.length === 0 ? (
          <NikiYarIsEmpty showAddressSelector={true} />
        ) : (
          <div
            className={styles["niki-yar-in-person-services__button-wrapper"]}
          >
            <Button
            ...
            >
              <div
                className={styles["niki-yar-in-person-services__button"]}
                onClick={() => {
                  setIsOpen(true);
                }}
              >

...
import styles from "./ProfileTasksInProgress.module.scss";
...
interface ProfileTasksInProgressProps {
  onViewTask?: (task: any) => void;
}

export default function ProfileTasksInProgress({ onViewTask }: ProfileTasksInProgressProps) {
  ...
  const inProgressTasks = useMemo(() => {
    let filteredTasks = inProgressTasksList;
  ...
  return (
    <div className={styles["profile-tasks-in-progress"]}>
      <div className={styles["profile-tasks-in-progress__content"]}>
        {/* فیلترها */}
        <div className={styles["profile-tasks-in-progress__filters"]}>
          <div className={styles["profile-tasks-in-progress__filter-group"]}>
            {/* فیلتر جستجو */}
            <TextField
              value={searchTerm}
              baseColor={{
        ...
        {isLoading && (
          <div className={styles["profile-tasks-in-progress__loading"]}>
            در حال بارگذاری...
          </div>
        )}

        {error && (
          <div className={styles["profile-tasks-in-progress__error"]}>
            خطا در بارگذاری اطلاعات: {error}
          </div>
        )}

        {!isLoading && !error && inProgressTasks.length === 0 && (
          <div className={styles["profile-tasks-in-progress__empty"]}>
            هیچ وظیفه‌ای در حال انجام وجود ندارد.
          </div>
        )}

        {!isLoading && !error && inProgressTasks.length > 0 && (
          <div className={styles["profile-tasks-in-progress__table-container"]}>
            <Pagination
              ...
              className={styles["profile-tasks-in-progress__pagination"]}

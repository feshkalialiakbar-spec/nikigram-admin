      ...
      {viewMode === 'list' && (
        <section className={styles.templateSelector}>
          <header className={styles.header}>
            <p className={styles.subtitle}>یکی از تمپلیت‌های زیر را برای ادامه فرآیند انتخاب کنید.</p>
          </header>

          <div className={styles.templatesGrid}>
            {templates.map(template => {
              const { template_detail: templateDetail, category_detail: categoryDetail } = template;
              const isSelected = activeTemplate?.template_detail.project_temp_id === templateDetail.project_temp_id;
                ...
                >
                  <div className={styles.cardHeader}>
                    <div className={styles.cardTitleGroup}>
                      <h3 className={styles.cardTitle}>{templateDetail.title || 'عنوان نامشخص'}</h3>
                      {isSelected && (
                  ...
                  <div className={styles.cardMeta}>
                    <div className={styles.metaRow}>
                      <span className={styles.metaLabel}>صندوق مربوطه:</span>
                      <span className={styles.metaValue}>{categoryDetail.fund_name || '—'}</span>
                    </div>
                    <div className={styles.metaRow}>
                      <span className={styles.metaLabel}>دسته‌بندی:</span>
                      <span className={styles.metaValue}>{categoryDetail.title || '—'}</span>
                    </div>
                    <div className={styles.metaRow}>
                      <span className={styles.metaLabel}>والد:</span>
                      <span className={styles.metaValue}>{categoryDetail.parent_title || '—'}</span>
                    </div>
                  </div>

                  <p className={styles.cardDescription}>
                    {renderTemplateDescription(templateDetail.description)}
                  </p>
          ...
          {hasMore && (
            <Button
              buttonClassName={styles.loadMoreButton}
              onClick={handleLoadMore}
              disabled={isLoading}
              bgColor="transparent"
            >
              <ArrowDown2 size={18} color="#0E315D" />
